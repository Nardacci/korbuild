...
function render(){
  ...
  const filtered=state.people.filter(p=>{
    const matchesText=!q || p.name.toLowerCase().includes(q) || (p.role||'').toLowerCase().includes(q);
    const matchesStatus=filter==='ALL' || (filter==='ACTIVE' ? p.active : !p.active);
    return matchesText && matchesStatus;
  });
  $('active-count').textContent=state.people.filter(p=>p.active).length;
  $('inactive-count').textContent=state.people.filter(p=>!p.active).length;
...
async function loadProfile(){
  ...
  if(error||!session?.user){window.location.href='index.html';return false;}
  ...
  const name=profile.name||session.user.user_metadata?.full_name||session.user.email?.split('@')[0]||'Owner';
...
async function loadPeople(){
  clearMessage();
  const {data,error}=await db.from('colaboradores').select('id,empresa_id,name,role,active,created_at,updated_at').eq('empresa_id',state.empresaId).order('name',{ascending:true});
  if(error){showMessage(`Unable to load people. ${error.message}`,'error');return;}
  state.people=data||[];render();
}
function openModal(person=null){
  state.editingId=person?.id||null;$('person-id').value=person?.id||'';$('person-name').value=person?.name||'';$('person-role').value=person?.role||'';$('person-active').checked=person?.active??true;
  $('modal-eyebrow').textContent=person?'EDIT PERSON':'NEW PERSON';$('modal-title').textContent=person?'Edit person':'Add person';$('save-person').textContent=person?'Save Changes':'Save Person';$('person-modal').classList.remove('hidden');setTimeout(()=>$('person-name').focus(),50);
}
function closeModal(){$('person-modal').classList.add('hidden');state.editingId=null;}
async function savePerson(event){
  event.preventDefault();
  const name=$('person-name').value.trim();const role=$('person-role').value.trim()||null;const active=$('person-active').checked;
  if(!name){showMessage('Name is required.','error');return;}
  const btn=$('save-person');btn.disabled=true;btn.textContent='Saving...';clearMessage();
  try{
    let error;
    if(state.editingId){({error}=await db.from('colaboradores').update({name,role,active,updated_at:new Date().toISOString()}).eq('id',state.editingId).eq('empresa_id',state.empresaId));}
    else{({error}=await db.from('colaboradores').insert({empresa_id:state.empresaId,name,role,active}));}
    if(error)throw error;
    closeModal();await loadPeople();showMessage(state.editingId?'Person updated successfully.':'Person added successfully.');
  }catch(error){showMessage(`We couldn't save this person. ${error.message||'Please try again.'}`,'error');}
  finally{btn.disabled=false;btn.textContent=state.editingId?'Save Changes':'Save Person';}
}
async function togglePerson(id){
  ...
  clearMessage();
  const {error}=await db.from('colaboradores').update({active:next,updated_at:new Date().toISOString()}).eq('id',id).eq('empresa_id',state.empresaId);
  if(error){showMessage(`We couldn't update this person. ${error.message}`,'error');return;}
  await loadPeople();showMessage(`${person.name} is now ${next?'active':'inactive'}.`);
}
...
$('add-person').addEventListener('click',()=>openModal());
$('close-modal').addEventListener('click',closeModal);$('cancel-modal').addEventListener('click',closeModal);$('person-modal').addEventListener('click',e=>{if(e.target.id==='person-modal')closeModal();});
$('person-form').addEventListener('submit',savePerson);$('search').addEventListener('input',render);$('status-filter').addEventListener('change',render);
...
async function init(){if(await loadProfile())await loadPeople();}
init();