...
function showAuthError(message) {
  ...
  $('employees').textContent = '—'; $('teams').textContent = '—'; $('pending').textContent = '—';
  $('period-value').textContent = '—'; $('period-label').textContent = 'Sign in required';
}

async function loadHome() {
  const { data: { session }, error: sessionError } = await db.auth.getSession();
  if (sessionError || !session?.user) { showAuthError('Please sign in to continue.'); return; }
  const user = session.user;
  const { data: profile, error: profileError } = await db.from('usuarios').select('id,name,empresa_id,active,empresas(name)').eq('id', user.id).maybeSingle();
  if (profileError || !profile) { showAuthError('Profile setup required'); return; }
  const company = profile.empresas?.name || 'KORbuild Demo';
  const name = profile.name || 'Owner';
  $('side-company').textContent = company; $('company-pill').textContent = company; $('user-name').textContent = name; $('user-email').textContent = user.email || '';
  ...
  $('employees').textContent = people.count ?? 0; $('teams').textContent = teams.count ?? 0; $('pending').textContent = evaluations.count ?? 0;
  if (period.data) {
    const start = new Date(period.data.start_date + 'T00:00:00'), end = new Date(period.data.end_date + 'T00:00:00');
  ...
  } else { $('period-value').textContent = '—'; $('period-label').textContent = 'No period prepared'; }
}

db.auth.onAuthStateChange((event) => { if (event === 'SIGNED_OUT') window.location.href = 'index.html'; });
$('logout').addEventListener('click', async () => { await db.auth.signOut(); window.location.href = 'index.html'; });
$('start-setup')?.addEventListener('click', () => { window.location.href = 'setup.html'; });
$('prepare').addEventListener('click', () => { window.location.href = 'setup.html'; });
loadHome();
