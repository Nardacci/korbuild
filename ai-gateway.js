(function(){
  ...
  if(!cfg||!cfg.url||!cfg.publishableKey||!window.supabase||typeof window.supabase.createClient!=='function'){
    console.error('[KORbuild AI] invalid Supabase configuration',cfg);
    return;
  }
  ...
  async function buildAuthorizedContext(){
    ...
    try{
      ...
      if(!uid)throw new Error('session: no authenticated user');
      stage='profile';
      ...
      if(!profile?.empresa_id)throw new Error('profile: empresa_id not found');
      ...
      if(periodRes.error)throw new Error('period: '+periodRes.error.message);
      if(insightRes.error)throw new Error('insights: '+insightRes.error.message);
      ...
      if(period){
        ...
        if(ids.length){
          ...
          if(oe)throw new Error('occurrences: '+oe.message);
          (occ||[]).forEach(o=>{
            const qty=Math.max(0,Number(o.quantity)||0);const sign=o.tipos_ocorrencia?.occurrence_type==='NEGATIVA'?-1:1;const score=sign*Math.abs(Number(o.points)||0)*qty;const cur=scores.get(o.lancamento_id)||{score:0,count:0};cur.score+=score;cur.count+=qty;scores.set(o.lancamento_id,cur);
          });
        }
      }
      stage='context assembly';
      const rows=[];const byPerson=new Map();people.filter(p=>p.active===true).forEach(p=>{const row={person:p,score:0,count:0};rows.push(row);byPerson.set(p.id,row);});
      ...
      setStatus('Context error · '+stage);
      console.error('[KORbuild AI] authorized context failed at '+stage,error);
      throw error;
    }
  }
  ...
  async function askViaGateway(text){
    ...
    if(!response.ok){setStatus('Gateway response · HTTP '+response.status);throw new Error(payload.error||'gateway_error');}
  ...
  async function init(){
    await waitForDom();
    messages=$('kor-ai-messages');
    if(!messages){setStatus('AI DOM error · messages not found');return;}
    setStatus('Gateway initialized · ready');
