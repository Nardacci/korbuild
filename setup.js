...
const state = { step: 2, maxPoints: 500, pointValue: 1.00, frequency: 'WEEKLY', startDay: '1', endDay: '6', preparation: 'MANUAL' };
...
function render() {
  ...
  if (state.step === 6) $('next').style.display = 'none';
}

function collect() {
  if (state.step === 2) { state.maxPoints = Number($('max-points').value); state.pointValue = Number($('point-value').value); }
  if (state.step === 3) { state.frequency = $('frequency').value; state.startDay = $('start-day').value; state.endDay = $('end-day').value; state.preparation = document.querySelector('input[name="preparation"]:checked')?.value || 'MANUAL'; }
}
...
$('go-people')?.addEventListener('click', () => { state.step = 5; render(); });
