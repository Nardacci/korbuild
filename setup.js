...
async function loadWorkspace(){const{data:{session}}=await db.auth.getSession();if(!session?.user){window.location.href='index.html';return}const{data:profile}=await db.from('usuarios').select('empresa_id,empresas(name)').eq('id',session.user.id).maybeSingle();if(profile?.empresas?.name)$('workspace-name').textContent=profile.empresas.name;const name=session.user.user_metadata?.full_name||session.user.email?.split('@')[0]||'Owner';$('menu-user-name').textContent=name;$('menu-full-name').textContent=name;$('menu-user-email').textContent=session.user.email||'';$('menu-full-email').textContent=session.user.email||'';$('user-avatar').textContent=name.charAt(0).toUpperCase()}
