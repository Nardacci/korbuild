...
async function load(){const {data:d,error}=await c.rpc('get_korbuild_commercial_admin_data');if(error||!d){$('denied').classList.remove('hidden');return;}data=d;$('admin-content').classList.remove('hidden');const s=d.settings||{};$('base-setup-fee').value=s.base_setup_fee??'';$('monthly-price').value=s.monthly_price??'';$('currency').value=s.currency||'USD';$('base-preview').textContent='Base setup: '+money(s.base_setup_fee,s.currency);renderCompanies();renderAccessCompanies();}
