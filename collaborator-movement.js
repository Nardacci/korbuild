...
function periodLabel(p){return t('Week')+' '+p.week_number+' · '+fmtDate(p.start_date)+' → '+fmtDate(p.end_date);}
...
function populatePeriods(){
 const sel=$('period-filter');sel.innerHTML=`<option value="ALL">${t('All periods')}</option>`+state.periods.map(p=>'<option value="'+p.id+'">'+esc(periodLabel(p))+'</option>').join('');
}
function buildRows(){
 ...
 state.occurrences.forEach(o=>{
  ...
  const existing=row.occurrences.get(typeKey)||{name:type.name||t('Occurrence'),quantity:0,points:point,total:0};existing.quantity+=qty;existing.total+=total;row.occurrences.set(typeKey,existing);row.total+=total;
 });
...
function render(){
 ...
 const box=$('movement-list');if(!rows.length){box.innerHTML=`<div class="empty">${t('No collaborator movement found for the selected filters.')}</div>`;return}
...
function renderPerson(p){
 ...
 return '<article class="collaborator-card"><div class="collaborator-head"><div class="person-avatar">'+esc(initials(p.person.name))+'</div><div class="collaborator-info"><h2>'+esc(p.person.name)+'</h2><p>'+esc(meta)+'</p></div><div class="movement-total"><small>'+t('TOTAL MOVEMENT')+'</small><strong class="'+(p.total<0?'negative':'positive')+'">'+pts(p.total)+'</strong></div></div>'+p.periods.map(renderPeriod).join('')+'</article>';
}
function renderPeriod(r){
 ...
 return '<div class="period-block"><div class="period-head"><div class="period-label"><small>'+t('EVALUATION PERIOD')+'</small><strong>'+esc(periodLabel(r.period))+'</strong></div><div class="period-movement '+movementClass+'">'+pts(r.total)+'</div></div><table class="occurrence-table"><thead><tr><th>'+t('OCCURRENCE')+'</th><th>'+t('QUANTITY')+'</th><th>'+t('MOVEMENT')+'</th></tr></thead><tbody>'+occ.map(o=>'<tr><td class="occ-name">'+esc(o.name)+'</td><td>'+o.quantity+'</td><td class="occ-points '+(o.total<0?'negative':'positive')+'">'+pts(o.total)+'</td></tr>').join('')+'</tbody></table></div>';
}
...
(async()=>{if(await profile())try{await load()}catch(e){console.error(e);showError(`${t('Unable to load collaborator movement.')} ${e.message||''}`);}})();
