...
(function applyKORbuildShell(){
 const run=()=>{ applyKORbuildVersion(); const sidebar=document.querySelector('aside.sidebar'); if(!sidebar)return;
  ...
  const nav=sidebar.querySelector('nav'); if(nav){const path=(location.pathname.split('/').pop()||'home.html').toLowerCase();const is=files=>files.includes(path);const active={dashboard:is(['home.html','']),evaluations:is(['evaluations.html']),periods:is(['periods.html']),schedule:is(['schedule.html','schedule-form.html']),bonusSettlement:is(['bonus-settlement.html']),collaboratorMovement:is(['collaborator-movement.html']),workUnits:is(['work-units.html','work-units-form.html']),teams:is(['teams.html','teams-form.html']),people:is(['people.html','people-form.html']),occurrences:is(['occurrences.html','occurrence-form.html']),payrollSettings:is(['payroll-settings.html']),weeklyPayments:is(['weekly-payments.html']),customerCalendar:is(['customer-schedule.html','customer-appointment-form.html']),customers:is(['customers.html','customer-form.html']),reminderSettings:is(['reminder-settings.html'])};const recordsActive=active.workUnits||active.teams||active.people||active.occurrences;const reportsActive=active.bonusSettlement||active.collaboratorMovement;const payrollActive=active.payrollSettings||active.weeklyPayments;const customerActive=active.customerCalendar||active.customers||active.reminderSettings;
   nav.innerHTML=`<a class="nav-item ${active.dashboard?'active':''}" href="home.html"><span>⌂</span>Dashboard</a><a class="nav-item ${active.evaluations?'active':''}" href="evaluations.html"><span>✓</span>Evaluations</a><a class="nav-item ${active.periods?'active':''}" href="periods.html"><span>◷</span>Periods</a><a class="nav-item ${active.schedule?'active':''}" href="schedule.html"><span>▤</span>Schedule</a><div id="records-group" class="nav-group" style="margin:2px 0"><button id="records-toggle" class="nav-group-title" type="button" aria-expanded="${recordsActive?'true':'false'}" style="width:100%;display:flex;align-items:center;gap:12px;padding:11px 12px;border:0;border-radius:9px;background:transparent;color:#40506a;font:500 13px Inter,system-ui,sans-serif;text-align:left;cursor:pointer"><span id="records-chevron" style="width:16px;text-align:center;font-size:12px;color:#6b7890">${recordsActive?'⌃':'⌄'}</span>Records</button><div id="records-items" class="nav-group-items" style="display:${recordsActive?'block':'none'};padding-left:8px"><a class="nav-item ${active.workUnits?'active':''}" href="work-units.html"><span>▣</span>Work Units</a><a class="nav-item ${active.teams?'active':''}" href="teams.html"><span>◇</span>Teams</a><a class="nav-item ${active.people?'active':''}" href="people.html"><span>◉</span>People</a><a class="nav-item ${active.occurrences?'active':''}" href="occurrences.html"><span>!</span>Occurrences</a></div></div><div class="nav-group" style="margin:2px 0"><button id="reports-toggle" class="nav-group-title" type="button" aria-expanded="${reportsActive?'true':'false'}" style="width:100%;display:flex;align-items:center;gap:12px;padding:11px 12px;border:0;border-radius:9px;background:transparent;color:#40506a;font:500 13px Inter,system-ui,sans-serif;text-align:left;cursor:pointer"><span id="reports-chevron" style="width:16px;text-align:center;font-size:12px;color:#6b7890">${reportsActive?'⌃':'⌄'}</span>Reports</button><div id="reports-items" class="nav-group-items" style="display:${reportsActive?'block':'none'};padding-left:8px"><a class="nav-item ${active.bonusSettlement?'active':''}" href="bonus-settlement.html"><span>▥</span>Bonus Settlement</a><a class="nav-item ${active.collaboratorMovement?'active':''}" href="collaborator-movement.html"><span>↕</span>Collaborator Movement</a></div></div><div class="nav-group" style="margin:2px 0"><button id="payroll-toggle" class="nav-group-title" type="button" aria-expanded="${payrollActive?'true':'false'}" style="width:100%;display:flex;align-items:center;gap:12px;padding:11px 12px;border:0;border-radius:9px;background:transparent;color:#40506a;font:500 13px Inter,system-ui,sans-serif;text-align:left;cursor:pointer"><span id="payroll-chevron" style="width:16px;text-align:center;font-size:12px;color:#6b7890">${payrollActive?'⌃':'⌄'}</span>Payroll</button><div id="payroll-items" class="nav-group-items" style="display:${payrollActive?'block':'none'};padding-left:8px"><a class="nav-item ${active.weeklyPayments?'active':''}" href="weekly-payments.html"><span>$</span>Weekly Payments</a><a class="nav-item ${active.payrollSettings?'active':''}" href="payroll-settings.html"><span>⚙</span>Payroll Settings</a></div></div><div class="nav-group" style="margin:2px 0"><button id="customer-toggle" class="nav-group-title" type="button" aria-expanded="${customerActive?'true':'false'}" style="width:100%;display:flex;align-items:center;gap:12px;padding:11px 12px;border:0;border-radius:9px;background:transparent;color:#40506a;font:500 13px Inter,system-ui,sans-serif;text-align:left;cursor:pointer"><span id="customer-chevron" style="width:16px;text-align:center;font-size:12px;color:#6b7890">${customerActive?'⌃':'⌄'}</span>Customer Service</button><div id="customer-items" class="nav-group-items" style="display:${customerActive?'block':'none'};padding-left:8px"><a class="nav-item ${active.customerCalendar?'active':''}" href="customer-schedule.html"><span>▦</span>Calendar</a><a class="nav-item ${active.customers?'active':''}" href="customers.html"><span>☺</span>Clients</a><a class="nav-item ${active.reminderSettings?'active':''}" href="reminder-settings.html"><span>✉</span>Reminders</a></div></div>`;
   const toggle=document.getElementById('records-toggle'),items=document.getElementById('records-items'),chevron=document.getElementById('records-chevron');if(toggle&&items&&chevron){toggle.addEventListener('click',()=>{const open=toggle.getAttribute('aria-expanded')==='true';toggle.setAttribute('aria-expanded',String(!open));items.style.display=open?'none':'block';chevron.textContent=open?'⌄':'⌃';});}
     ...
     const payrollToggle=document.getElementById('payroll-toggle'),payrollItems=document.getElementById('payroll-items'),payrollChevron=document.getElementById('payroll-chevron');if(payrollToggle&&payrollItems&&payrollChevron){payrollToggle.addEventListener('click',()=>{const open=payrollToggle.getAttribute('aria-expanded')==='true';payrollToggle.setAttribute('aria-expanded',String(!open));payrollItems.style.display=open?'none':'block';payrollChevron.textContent=open?'⌄':'⌃';});}
     const customerToggle=document.getElementById('customer-toggle'),customerItems=document.getElementById('customer-items'),customerChevron=document.getElementById('customer-chevron');if(customerToggle&&customerItems&&customerChevron){customerToggle.addEventListener('click',()=>{const open=customerToggle.getAttribute('aria-expanded')==='true';customerToggle.setAttribute('aria-expanded',String(!open));customerItems.style.display=open?'none':'block';customerChevron.textContent=open?'⌄':'⌃';});}
 }
