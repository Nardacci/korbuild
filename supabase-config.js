...
<div id="collaborators-group" class="nav-group" style="margin:2px 0">${groupBtn('collaborators','Collaborators',groupActive.collaborators)}<div id="collaborators-items" class="nav-group-items" style="display:${groupActive.collaborators?'block':'none'};padding-left:8px">${navItem(active.people,'people.html','◉','People')}${navItem(active.teams,'teams.html','◇','Teams')}${navItem(active.workUnits,'work-units.html','▣','Work Units')}</div></div>
...
<div id="schedule-group" class="nav-group" style="margin:2px 0">${groupBtn('schedule','Schedule',groupActive.schedule)}<div id="schedule-items" class="nav-group-items" style="display:${groupActive.schedule?'block':'none'};padding-left:8px">${navItem(active.schedule,'schedule.html','▤','Schedule')}<div id="customer-service-group" class="nav-group" style="margin:2px 0">${groupBtn('customer-service','Customer Service',groupActive.customerService)}<div id="customer-service-items" class="nav-group-items" style="display:${groupActive.customerService?'block':'none'};padding-left:8px">${navItem(active.customerCalendar,'customer-schedule.html','▦','Calendar')}${navItem(active.customers,'customers.html','☺','Clients')}${navItem(active.reminderSettings,'reminder-settings.html','✉','Reminders')}</div></div></div></div>
<div id="payment-group" class="nav-group" style="margin:2px 0">${groupBtn('payment','Payment',groupActive.payment)}<div id="payment-items" class="nav-group-items" style="display:${groupActive.payment?'block':'none'};padding-left:8px">${navItem(active.weeklyPayments,'weekly-payments.html','$','Weekly Payments')}${navItem(active.payrollSettings,'payroll-settings.html','⚙','Payroll Settings')}</div></div>`;
   ['collaborators','bonus','schedule','customer-service','payment'].forEach(id=>{const toggle=document.getElementById(id+'-toggle'),items=document.getElementById(id+'-items'),chevron=document.getElementById(id+'-chevron');if(toggle&&items&&chevron){toggle.addEventListener('click',()=>{const open=toggle.getAttribute('aria-expanded')==='true';toggle.setAttribute('aria-expanded',String(!open));items.style.display=open?'none':'block';chevron.textContent=open?'⌄':'⌃';});}});
 }
